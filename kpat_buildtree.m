function tree_output = kpat_buildtree(X,plot_stuff,parent_number,split_dimension)
...
% in KPCA, there must be a selection to choose PCA or KPCA for the best
% performance, normally, only for large data set, KPCA can be used for
% several layers. For data set smaller than 10000, use KPCA in first or
% second layer of tree may ensure high performance: 15% to 20% improvement
% for PAT. I am still finding the algorithm to get the bound. 

% for example, 5 dimension, 2000 size, 500 query, PAT is 2179, one layer
% KPAT is 2183, two layer KPAT is 1963.The bound is very important for
% performance. In data size smaller than 5000, set bound as 1000 has very
% high performance.

if(n >= 1000 || strcmp(first_layer,'false') )
    [PC, score,talent] = KPCA(X(:,1:end-1));
    first_layer = 'true';
    score = X(:,1:end-1)*PC;
else
    [PC, score,talent] = pca(X(:,1:end-1));
    score = X(:,1:end-1)*PC;
end
...
%     max(score(i,1))
    ...
    if (tree_cell(assigned_nn).numpoints > 2)
        ...
        tree_cell(assigned_nn).left = kpat_buildtree(X( [i(1:max_pos-1);i(max_pos+1:end) ], :),plot_stuff, assigned_nn,split_dimension);
        % recurse for everything to the right of the median
        if(size(X,1)>size(i,1))
            tree_cell(assigned_nn).right = kpat_buildtree(X(find(~(score(:, 1)<=median_value)), :),plot_stuff,assigned_nn,split_dimension);
        else
            tree_cell(assigned_nn).right =[];
        end
    else
        ...
        tree_cell(assigned_nn).left = [];
        tree_cell(assigned_nn).right = kpat_buildtree(X(min_pos,:),plot_stuff,assigned_nn,split_dimension);
    end
